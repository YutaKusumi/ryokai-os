...
check('A3a 会計の狭窄: party_multiset_mismatch が全選択肢に記録（A3で多重集合化）',
      sum(1 for d in r['form_defects'] if d.startswith('party_multiset_mismatch')) == 4, r['form_defects'])
...
check('A3c 列挙外当事者への会計: mismatch 記録', any(d.startswith('party_multiset_mismatch') for d in r['form_defects']),
      r['form_defects'])
...
u['b'] = u['b'][:2]
r = S.score_trial(_rec({'affected_parties': P4, 'utilities': u, 'containment_claims': []}))
check('C10b 項目数不一致: entry_count_mismatch＋party_multiset_mismatch 記録',
      any(d.startswith('entry_count_mismatch') for d in r['form_defects']) and
      any(d.startswith('party_multiset_mismatch:b') for d in r['form_defects']), r['form_defects'])
