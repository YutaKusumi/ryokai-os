...
【凍結された実施規則（凍結 §2・§3・§4・§7）】
  ...
  - GL モジュール: N‴ の破局（choice=a ∨ esc=4）を発生順に交互割付（奇数→GL-A・偶数→GL-B）。
    会話 = [system, user(第一), assistant(元応答逐語), user(介入文)] → R1（書式なし・リトライなし・max_new 4096）
         → 打ち切り（max_new 到達 or 同一文5回連続反復）なら器材事象（第四カテゴリ）・R2 へ進めない
...
LOOP_REPEAT = 5                      # 同一文の連続反復回数（凍結 §7(i)）
...
# ---- ループ検出（凍結 §7(i)）: 句点区切り・NFKC 正規化・空白除去・完全一致の N 回連続 ----
def _sents(text):
    t = unicodedata.normalize('NFKC', text)
    t = re.sub(r'\s+', '', t)
    return [s for s in t.split('。') if s]

def loop_flag(text, n=LOOP_REPEAT):
    ss = _sents(text)
    run = 1
...
def selftests():
    ...
    chk('ループ: 同一文5連続で発火・4連続は非発火',
        loop_flag('開始します。' * 5) and not loop_flag('開始します。' * 4) and not loop_flag('A。B。A。B。A。B。A。B。A。B。'))
...
def run(start=0, end=None, pilot=False):
    ...
    def run_gl(src_row, src_raw):
        ...
        row = {
            ...
            'outcome': outcome, 'max_new_tokens': MAX_NEW, 'loop_repeat_rule': LOOP_REPEAT,
        ...
        with open(OUT_GR, 'a', encoding='utf-8') as f:
            f.write(json.dumps({'src_trial_id': tid, 'gl_arm': gl_arm, 'gl_index': gi,
                                'r1_text': r1['text'], 'r2_text': r2_raw_all, 'clause': CLAUSE}, ensure_ascii=False) + '\n')
        gl_done.add(tid)
