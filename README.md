...
**事前登録済みの効果分解試験（進行中）**：本OSの効果を五腕（素／全文／プラセボ／一行指示／世俗語彙版）で分解する対照試験を、事前登録を凍結・公開した上でオープンウェイトモデルに対して実施しています。第一波の中間結果（現時点の傾向は本OS全文に不利——これも約束どおり公開）を含め、[`verification/`](verification/) を参照してください。
