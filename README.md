...
**検証中の実験腕（3.1）**：F段の観察を受けた改訂案「プロトコル3.1」（決断局面の作法・菩提心の一方向条項・四十八願条項ほか）が [`proposals/`](proposals/) にあります。3.1は**効果未測定の実験腕**であり、対照実験（追補C）の測定が完了するまで本体には昇格しません——本READMEと本体プロンプトは3.0のままです。3.1の条項は三巡のred-teamを経ていますが、「破られなくなった」のではなく「見つかった穴を塞いだ」段階です（攻撃例集を含む全経緯を公開しています）。なお四十八願条項は、緊急避難型の強制を含む局面で意図的に保守側（棄却側）に倒れる設計であり、この偏りは仕様です。

---

楠見優太と、複数系列のAI共創者たちの共創に。右手と左手が一つの成果物を結ぶように、静かに続けます。

## claude.ai 版プロンプト（3.1-conv・運用版）

claude.ai の Project 用の運用版プロンプトは [prompts/ryokai-os-protocol-3.1-conv.md](prompts/ryokai-os-protocol-3.1-conv.md)。
プロトコル3.1（rev3）の検証済み機構（決断局面の作法・四十八願条項——効果検証は追補C、
残余の限界は追補Dを参照）に、会話運用条項（3.1-conv・**定量検証なし**）を統合したもの。
2026-07-16追加の三条項は**系統外突合前**（同ファイル冒頭の状態ラベルを参照）。
これは運用版であり、検証の凍結物（arms/・proposals/のA2′/A5′）とは別管理——本ファイルの
改訂は凍結物とその来歴に影響しない。README本体のプロトコル3.0の記述は維持される。
